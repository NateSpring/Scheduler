  ...
  return (
    <>
      <div className="flex flex-row items-center justify-between">
        <PageTitle>FINAL ASSEMBLY</PageTitle>
      </div>
                    ...
                    className={`absolute flex justify-center items-center z-2 h-8 shadow-xl font-bold text-xl text-white text-center rounded-l-lg rounded-r-xl ${progressLength(
                      getSchedulerCapacity(
                        ...
                        operatorData.filter((operator) => {
                          return (
                            operator.operator !== "" && operator.dept == 12
                          );
                        }),
                        hopper.filter((build) => {
                          return build.current_dept == 12;
                        })
                      )
                    )}`}
                  >
                    {getSchedulerCapacity(
                      12,
                      operatorData.filter((operator) => {
                        return operator.operator !== "" && operator.dept == 12;
                      }),
                      hopper.filter((build) => {
                        return build.current_dept == 12;
                      })
        ...
        <TabPanel>
          <div className="grid gap-1 md:grid-cols-2 xl:grid-cols-3 p-1 w-full">
              ...
              <Pie
                data={piData(
                  ...
                  defectLog.filter((defect) => {
                    return defect.defect_dept == 12 || defect.from_dept == 12;
                  }),
                  completedTakts,
                  12
                )}
            ...
            <ChartCard size="text-xl" title="Daily Takt Tracker">
              ...
              <Line
                data={taktTrackerData("today", completedTakts, hopper, 12)}
                options={{
                  options: {
                    responsive: true,
                    tooltips: {
        ...
        <TabPanel>
          <div className="grid gap-1 md:grid-cols-2 xl:grid-cols-3 p-1 w-full">
            ...
            <ChartCard size="text-xl" title="Weekly Takt Tracker">
              ...
              <Line
                data={taktTrackerData("week", completedTakts, hopper, 12)}
                options={{
                  options: {
                    responsive: true,
                    tooltips: {
